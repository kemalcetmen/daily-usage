...
const Mic = ({
  ...
  setDisableButton}: Props) => {
    ...
    } = useSpeechRecognition({transcribing:disableButton})

    useEffect(() => {
        if(listening && disableButton) setTextToTranslate?.(transcript)
    },[listening, setTextToTranslate, transcript, disableButton])
